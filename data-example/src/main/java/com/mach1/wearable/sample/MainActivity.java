package com.mach1.wearable.sample;
...
import com.mach1.wearable.sample.viewmodels.ConnectionState;
import com.mach1.wearable.sample.viewmodels.Destination;
import com.mach1.wearable.sample.viewmodels.SessionViewModel;
...
public class MainActivity extends AppCompatActivity implements PreferenceFragmentCompat.OnPreferenceStartFragmentCallback {
    ...
    public boolean onPreferenceStartFragment(PreferenceFragmentCompat caller, Preference pref) {
        ...
        Log.i("foobar", "onPrefStart " + key + " " + pref.getFragment());

        if ("device_info".equals(key)) {
            navigateTo(R.id.deviceInfoFragment);
        } else if ("device_properties".equals(key)) {
            navigateTo(R.id.devicePropertiesFragment);
        } else if ("wearable_device_info".equals(key)) {
            navigateTo(R.id.wearableDeviceInfoFragment);
        } else if ("wearable_sensor_info".equals(key)) {
            navigateTo(R.id.sensorInfoListFragment);
        } else if ("wearable_sensor_config".equals(key)) {
            navigateTo(R.id.sensorConfigFragment);
        } else if ("wearable_sensor_data".equals(key)) {
            navigateTo(R.id.sensorDataFragment);
        } else if ("wearable_gesture_config".equals(key)) {
            navigateTo(R.id.gestureConfigFragment);
        } else if ("wearable_gesture_data".equals(key)) {
            navigateTo(R.id.gestureDataFragment);
        } else if ("available_sensors".equals(key)) {
            navigateTo(R.id.availableSensorsFragment);
        } else if ("available_gestures".equals(key)) {
            navigateTo(R.id.availableGesturesFragment);
        } else if (key.startsWith("sensor_info_")) {
            final Byte sensorValue = Byte.parseByte(key.substring("sensor_info_".length()));
            final SensorType sensorType = SensorType.fromData(sensorValue);
            final Bundle args = new Bundle();
            args.putSerializable(SensorInfoFragment.ARG_SENSOR_TYPE, sensorType);

            Navigation.findNavController(this, R.id.my_nav_host_fragment)
                .navigate(R.id.sensorInfoFragment, args);
        }
